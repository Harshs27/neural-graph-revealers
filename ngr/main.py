...
# local imports
import ngr.utils.neural_view as neural_view
import ngr.utils.data_processing as dp


######################################################################
# Functions for NGR graph recovery
######################################################################
...
def forward_NGR(X, model, S, structure_penalty='hadamard', lambd=0.1):
    ...
    if lambd > 0:
        ...
        if structure_penalty=='hadamard':
            ...
            structure_loss_diag = torch.linalg.norm(prod_W*Sg, ord=1)
            # Constraint on making the connections sparse
            structure_loss_W_sparsity = torch.linalg.norm(prod_W, ord=1)
        else:
...
def recover_graph(
    ...
    """Learn the underlying sparse graph using NGR. Fit a MLP
    to learn the data representation from X -> X. 
    ...
    for _k, (train, test) in enumerate(kf.split(X)):
        ...
        if VERBOSE: print(f'Initializing the NGR model')
        ...
        for e in range(epochs):
            ...
            _, loss_train, reg_loss_train, struct_loss_train = forward_NGR(
                X_train_batch, 
                model, 
                S,
                structure_penalty,
            ...
            with torch.no_grad(): # prediction on test 
                _, loss_test, reg_loss_test, struct_loss_test = forward_NGR(
                    X_test_batch, 
                    model, 
                    S,
                    structure_penalty, 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#%%% Functions for the obtained NGR model that is an instance of Neural Graphical models
